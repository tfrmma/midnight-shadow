...
Oracle is overdue. Three positions are liquidatable in shadow reality across $16.7K in first-touch MEV, but the protocol sees none of it. When the oracle fires, this window is measured in blocks.
...
The first-touch transaction is therefore disproportionately valuable: it both triggers crystallization and locks in the seizure discount before any competitor can act.
...
```
maxDebt   = Σ cᵢ · pᵢ · LLTVᵢ           (per whitepaper eq.3)
h-LTV     = debt / maxDebt
liquidatable when h-LTV > 1.0
```

**Liquidation Incentive Factor** (per whitepaper eq.4):
...
Multi-collateral positions use a blended LIF weighted by shadow collateral value contribution.

**Minimum seizure** to restore health (post-crystallization):

```
Δ_min = (D_remaining − maxDebt_shadow) / (1 − blended_LLTV · LIF)

first_touch_mev = Δ_min · (LIF − 1) − gas
```

**Infeasibility**: when `Δ_min ≥ D_remaining` or residual collateral after seizure falls below `rcfThreshold` (dust floor), full liquidation is allowed per whitepaper §4.3. These positions show `FULL` in the MEV column.

**Oracle lag direction**: only downward price moves create cliff risk for lenders. If oracle lags behind a price rally, collateral is worth more than the protocol knows. The `Lag↓` column shows downward divergence only.
 ...
 live: stub (Midnight not yet on mainnet)                             │
     ...
     └────────────────────────────┬────────────────────────────────--─┘
              ...
              per position:
                ...
                latent_bad_debt, Δ_min, first_touch_mev,
                lif_max (blended), cliff_imminent,
...
|---|---|---|---|---|---|---|
| weETH/USDC-Sep26 | 20 weETH (×1.001) | 0.86 | 0.50 | 1.111 | ~86% | ~104% |
| wstETH/USDC-Sep26 | 60 wstETH (×1.07) | 0.80 | 0.50 | 1.111 | ~88% | ~106% |
| ETH+wstETH/USDC-Sep26 | 30 ETH + 20 wstETH | 0.86/0.80 | 0.50 | ~1.099 | ~88% | ~107% |

The third position demonstrates multi-collateral blended LIF. All use the LLTV tiers from Morpho Blue's fixed set.
